...
colnames(table2) <- c('ataque cardíaco\n(0-30 dias)', 'sem evento\n(0-30 dias)',
                      'ataque cardíaco\n(0-365 dias)', 'sem evento\n(0-365 dias)')

knitr::kable(table2, align = 'c',
             caption = 'Estatística descritiva para o estudo sobre stent') %>% column_spec(1:length(colnames(table2)), width = "6em")
```
...
knitr::kable(table4, align = 'r', 
             caption = 'Variáveis e suas descrições para o conjunto de dados email50') %>% column_spec(1:length(colnames(table4)), width = "15em")
```
...
knitr::kable(head(county, 7), align = 'c', 
             caption = 'Sete linhas do conjunto de dados condado.')  %>% column_spec(1:length(colnames(head(county, 7))), width = "3em")
```
...
knitr::kable(table6, align = 'r', 
              caption = 'Variáveis e suas descrições para o banco de dados condado.') %>% column_spec(1:length(colnames(table6)), width = "15em")
```
...
***
```{exercise, echo=T, label = 'identifyingThePopulationForTwoQuestionsInPopAndSampSubsection'}
...
***
```{exercise, echo=T, label = 'sunscreenLurkingexample, echo=T'}
...
```{example, echo=T, label = 'wtdMeanOfIncome'}
...
***
```{exercise, echo=T, label = 'numCharWhichIsMoreRobust'}
...
ggplot(data = MLB) + 
  ...
  labs(x = 'Salário (em milhões de dólares)', y = "")
...
ggplot(data = MLB) + 
  ...
  labs(x = 'log(Salário) (em milhões de dólares)', y = "")
...
```{r countyIntensityMaps4, fig.cap = 'Mapa de intensidade da renda familiar média (1000 dólares).', out.width = '50%'}
...
```{example, echo=T, label = 'weighingRowColumnProportions'}
...
***
```{exercise, echo=T}
Olhe para as Tabelas \@ref(tab:rowPropSpamNumber) e \@ref(tab:colPropSpamNumber). O que seria mais útil para alguém na esperança de identificar e-mails de spam usando a variável `número`?^[As proporções da coluna na Tabela~\@ref{tab:colPropSpamNumber} provavelmente será mais útil, o que torna mais fácil ver que e-mails com números pequenos são spam cerca de 5,9\% do tempo (relativamente raro). Vimos também que cerca de 27,1\% de e-mails sem números são spam e 9,2\% de e-mails com números grandes são spam.]
```
...
knitr::kable(table13, caption = 'Nesta tabela, a renda familiar média (em 1000 dólares) de uma amostra aleatória de 100 municípios que ganharam população entre 2000-2010. Rendimentos médios de uma amostra aleatória de 50 municípios que não tiveram ganho de população são mostrados também')
```
...
g1 <- ggplot(side_side) + 
  ...
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1)) + 
  labs(x = 'Mudança na População', y = 'Renda Média ($1000)', color = "")

g2 <- ggplot(side_side) + 
  geom_histogram(aes(x = pov, fill = pop), alpha = 0.7, color = 'white') +
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1)) + 
  labs(x = 'Mudança na População', y = 'Renda Média ($1000)', fill = "")
...
***
```{exercise, echo=T, label = 'comparingPriceByTypeExercise'}
...
```{example, echo=T, label = 'classRightLeftSideApple'}
...
```{example, echo=T, label = 'discriminationResultsWhatIsConvincingEvidence'}
...
***
```{exercise, echo=T, label = 'sampleDifferenceInMaleAndFemaleDiscrimination'}
