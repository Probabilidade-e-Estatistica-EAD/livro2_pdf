...
```{example, echo=T, label = 'probOf1Or2'}
...
```{example, echo=T, label = 'probOf2Ones'}
...
***
```{exercise, echo=T, label = 'randomProcessExercise'}
...
***
```{exercise, echo=T}
Estamos interessados na probabilidade de rolar um 1, 4, ou 5: (a) Explicar porque os resultados 1, 4, e 5 são disjuntos. (b) Aplicar a Regra da Adição para resultados disjuntos para determinar $P(1 ou 4 ou 5)$.^[(a) O processo aleatório é jogar um dado e, no máximo, um desses resultados pode surgir. Isso significa que eles são resultados disjuntos (b) $P(1 ou 4 ou 5$) = $P(1) + P(4) + P(5) = \frac{1}{6} + \frac{1}{6} + \frac{1}{6} = \frac{3}{6} = \frac{1}{2}$]
```
...
***
```{exercise, echo=T}
No banco de dados `email` da introdução, a variável _número_ descreveu se nenhum número, apenas um ou mais números pequenos, ou se pelo menos um grande número apareceu em um email. Dos 3.921 e-mails, 549 não tinham números, 2.827 tinham apenas um ou mais números pequenos e 545 tinham pelo menos um número grande: (a) Os resultados __nenhum__, __pequeno__, e __grande__ são  disjuntos? (b) Determinar a proporção de emails com valor __pequeno__ e __grande__, separadamente. (c) Use a Regra da Adição para resultados disjuntos para calcular a probabilidade de um e-mail selecionado aleatoriamente do conjunto de dados ter um número, pequeno ou grande.^[(a) Sim. Cada email é categorizado em apenas um nível de número. (b) Pequeno: $\frac{2827}{3921} = 0.721$. Grande: $\frac{545}{3921} = 0.139$. (c) $P(\text{pequeno ou grande}) = P(\text{pequeno}) + P(\text{grande}) = 0.721 + 0.139 = 0.860$.]
```
...
***
```{exercise, echo=T, label = 'exerExaminingDisjointSetsABD'}
...
***
```{exercise, echo=T, label = 'emailSpamNumberVennExer'}
...
knitr::kable(tab1, caption = 'Distribuição de probabilidade da soma de dois dados.', align = 'c') %>% column_spec(1, width = "5em")
...
***
```{exercise, echo=T, label = 'usHouseholdIncomeDistsExercise'}
...
```{example, echo=T, label = 'threeDice'}
...
***
```{exercise, echo=T, label = 'ex2Handedness'}
...
***
```{exercise, echo=T, label = 'ex5Handedness'}
...
knitr::kable(tab3, align = 'c', 
             caption = 'Tabela de contingência resumindo o conjunto de dados familia ensino')
```
...
```{example, echo=T, label = 'collegeProbOfParentsGivenStudentNot'}
...
knitr::kable(tab5, align = 'c', 
             caption = 'Distribuição de probabilidade conjunta para o conjunto de dados faculdade familia')
```
...
***
```{exercise, echo=T, label = 'familyCollegeProbOfParentsEqualNotGivenTeen'}
...
***
```{exercise, echo=T, label = 'whyCondProbSumTo1'}
...
***
```{exercise, echo=T, label = 'probDiedIfNotInoculated'}
...
***
```{exercise, echo=T, label = 'SmallpoxInoculationObsExpExercise'}
...
***
```{exercise, echo=T, label = 'condProbOfRollingA1AfterOne1'}
...
```{example, echo=T, label = 'exerciseForTreeDiagramOfStudentGettingAOnMidtermGivenThatSheGotAOnFinal'}
...
```{example, echo=T, label = 'probabilityOfBreastCancerGivenPositiveTestExample'}
...
***
```{exercise, echo=T, label = 'exerciseForParkingLotOnCampusBeingFullAndWhetherOrNotThereIsASportingEvent'}
...
***
```{exercise, echo=T, label = 'exerciseForParkingLotOnCampusBeingFullAndWhetherOrNotThereIsAnAcademicEvent'}
...
***
```{exercise, echo=T, label = 'exerciseForParkingLotOnCampusBeingFullAndWhetherOrNotThereIsNoEvent'}
...
***
```{exercise, echo=T, label = 'raffleOf30TicketsWWOReplacement'}
...
***
```{exercise, echo=T, label = 'followUpToRaffleOf30TicketsWWOReplacement'}
...
```{example, echo=T, label = 'bookStoreSales'}
...
```{example, echo=T, label = 'bookStoreRev'}
...
```{example, echo=T, label = 'revFromStudent'}
...
***
```{exercise, echo=T, label = 'elenaIsSellingATVAndBuyingAToasterOvenAtAnAuction'}
...
***
```{exercise, echo=T, label = 'explainWhyThereIsUncertaintyInTheSum'}
...
***
```{exercise, echo=T, label = 'expectedChangeInLeonardsStockPortfolio'}
...
```{r changeInLeonardsStockPortfolioFor36Months, out.width='100%', fig.cap = 'A mudança em um portfólio como o de Leonard para os 36 meses de 2009 a 2011, em que $6000 está no estoque do Google e $2000 está no da Exxon Mobil.'}
...
```{example, echo=T, label = 'sdOfJohnsCommuteWeeklyTime'}
...
***
```{exercise, echo=T, label = 'elenaIsSellingATVAndBuyingAToasterOvenAtAnAuctionVariability'}
...
```{example, echo=T, label = 'usHeights'}
...
```{example, echo=T, label = 'contDistProb'}
...
```{example, echo=T, label = 'probabilityOfExactly180cm'}
