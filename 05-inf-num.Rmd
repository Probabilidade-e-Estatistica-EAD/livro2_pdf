...
```{example, echo=T, label = "htForDiffInUCLAAndAmazonTextbookPrices"}
...
knitr::kable(head(births[,c(1:3, 7:9)], 4), align = "c", 
             caption = "Quatro casos do conjunto de dados bebe fumo.")
```
...
```{example, echo=T, label = "babySmokeHTForWeight"}
...
```{example, echo=T, label = "pictureOfPValueForEstimateOfDiffOfMeansOfBirthWeights"}
...
```{example, echo=T, label = "babySmokeHTForWeightComputePValueAndEvalHT"}
...
***
```{exercise, echo=T, label = "babySmokeHTIDingHowToDetectDifferences"}
...
***
```{exercise, echo=T, label = "htSetupForEvaluatingTwoExamVersions"}
...
***
```{exercise, echo=T, label = "conditionsForTDistForEvaluatingTwoExamVersions"}
...
```{example, echo=T, label = 'PowerFor100AtNeg3'}
...
```{example, echo=T, label = 'firstExampleForThreeStatisticsClassesAndANOVA'}
...
```{example, echo=T, label = "multipleComparisonExampleThatIncludesDiscussionOfClassrooms"}
...
***
```{exercise, echo=T, label = 'describePValueAreaForFDistributionInMLBOBPExample'}
...
***
```{exercise, echo=T, label = 'exerExaminingAnovaSummaryTableForMidtermData'}
...
```{example, echo=T, label = 'multipleComparisonsOfThreeStatClasses'}
